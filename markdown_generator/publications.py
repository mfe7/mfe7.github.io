...
publications = pd.read_csv(
    sheet_url, sep="\t", header=0, keep_default_na=False
)
...
for row, item in publications.iterrows():
    ...
    if month == "":
        ...
        month = str(datetime.datetime.strptime(item.Month, "%B").month).zfill(
            2
        )
    paper_id = item.Abbreviation
    filename = "{year}-{month}-{paper_id}".format(
        year=year, month=month, paper_id=paper_id
    )
    ...
    if "," in item.Authors:
        ...
        for author in authors:
            ...
            authors_str += (
                first_name.strip() + " " + last_name.strip() + " and "
            )
    ...
    # latex -> unicode
    authors_str = authors_str.replace(r"\"{o}", "ö")
    authors_str = authors_str.replace(r"\"{u}", "ü")
    authors_str = authors_str.replace(r"{\i}", "ı")
    authors_str = authors_str.replace(r"{\u{g}}", "ğ")
    authors_str = authors_str.replace(r"{\c{s}}", "ş")
    ...
    venue_dict = {
        ...
        "l4dc": "Learning for Dynamics and Control Conference (L4DC)",
    }
    ...
    include_on_website = (
        "true" if item["Include on Website"] == "Y" else "false"
    )
